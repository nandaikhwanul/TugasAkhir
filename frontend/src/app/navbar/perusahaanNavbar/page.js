...
export default function PerusahaanNavbar() {
  ...
  useEffect(() => {
    ...
    async function fetchProfileImage() {
      try {
        const res = await axios.get("http://localhost:5000/perusahaan/me", {
    ...
    async function fetchAlumni() {
      ...
      try {
        const res = await axios.get(
          `http://localhost:5000/perusahaan/alumni?q=${encodeURIComponent(debouncedSearchQuery)}`,
          {
            headers: {
                              ...
                              src={
                                alumni.foto_profil
                                  ? alumni.foto_profil.startsWith("http")
                                    ? alumni.foto_profil
                                    : `http://localhost:5000/uploads/alumni/${alumni.foto_profil}`
                                  : "/default-profile.png"
                              }
                              alt={alumni.name}
