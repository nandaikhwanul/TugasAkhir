...
async function incrementTrafficLowongan(lowonganId) {
  try {
    ...
    await fetch(`http://localhost:5000/lowongan/${lowonganId}/traffic`, {
      method: "POST",
      headers: {
...
async function toggleSaveLowongan(lowonganId) {
  try {
    const token = getTokenFromCookie();
    if (!token) throw new Error("Token tidak ditemukan");
    const res = await fetch("http://localhost:5000/alumni/me/toggle-save-lowongan", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        ...
        const resSaved = await fetch("http://localhost:5000/alumni/me/saved-lowongan", {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (resSaved.ok) {
                    ...
                    {item.perusahaan?.logo_perusahaan ? (
                      <img
                        src={item.perusahaan.logo_perusahaan.startsWith("http")
                          ? item.perusahaan.logo_perusahaan
                          : `http://localhost:5000${item.perusahaan.logo_perusahaan}`
                        }
