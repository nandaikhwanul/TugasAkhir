          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-6 pt-6 mb-4">
            <div className="text-[20px] text-[#222] font-semibold">
              ...
              <span className="text-[#6c757d] font-normal">
                ({rekomendasi.length} total)
              </span>
            </div>
            {/* Sorting dropdown dihapus sesuai instruksi */}
          </div>
          <div
          ...
          >
            ...
            {loading ? (
              ...
              rekomendasi.map((item) => {
                // Adaptasi struktur data agar sesuai tampilan
                const nama_lowongan = item.judul_pekerjaan || item.nama_lowongan || "-";
                const nama_perusahaan =
                  (item.perusahaan && item.perusahaan.nama_perusahaan) ||
                  item.perusahaan_nama ||
                  item.nama_perusahaan ||
                  "-";
                const logo_perusahaan =
                  (item.perusahaan && item.perusahaan.logo_perusahaan) ||
                ...
                return (
                  ...
                  >
                    {/* Logo, Info Utama, Info Tambahan, dan Apply Button */}
                    <div className="flex items-start mb-4 sm:mb-0 sm:mr-6 w-full sm:w-auto">
                      ...
                      <div className="w-16 h-16 rounded-xl bg-[#eaf7e6] flex items-center justify-center mr-4 flex-shrink-0 overflow-hidden border border-[#e0e0e0]">
                        {logo_perusahaan ? (
                          <img
                            src={
                              logo_perusahaan.startsWith("http")
                                ? logo_perusahaan
                                : `https://tugasakhir-production-6c6c.up.railway.app${logo_perusahaan}`
                            }
                            alt={nama_perusahaan || "Logo"}
