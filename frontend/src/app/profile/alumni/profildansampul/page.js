...
function getFotoSampulUrl(foto_sampul) {
  ...
  return `http://localhost:5000/uploads/alumni/sampul/${foto_sampul}`;
}
  ...
  const handleSampulChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        setSaving(true);
        setFormError("");
        const res = await fetch("http://localhost:5000/alumni/me/foto-sampul", {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleProfilChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        setSaving(true);
        setFormError("");
        const res = await fetch("http://localhost:5000/alumni/me/foto-profil", {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
          },
