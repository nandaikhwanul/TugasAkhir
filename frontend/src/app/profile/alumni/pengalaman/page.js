...
export default function PengalamanCard() {
  ...
  useEffect(() => {
    async function fetchPengalaman() {
      ...
      try {
        const res = await fetch("http://localhost:5000/pengalaman/me", {
          headers: {
  ...
  const handlePengalamanSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/pengalaman", {
        method: "POST",
        headers: {
      ...
      // Refetch pengalaman
      setLoadingPengalaman(true);
      const pengalamanRes = await fetch("http://localhost:5000/pengalaman/me", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
