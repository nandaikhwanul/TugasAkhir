...
async function likeForumPost(postId) {
  ...
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/like`, {
    method: "POST",
    headers: {
...
async function unlikeForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/unlike`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function saveForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/save`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function unsaveForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/unsave`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function pinForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/pin`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function unpinForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/unpin`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function highlightForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/highlight`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function unhighlightForumPost(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/posts/${postId}/unhighlight`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
async function createForumComment({ postId, content }) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch("http://localhost:5000/forum/comments", {
    method: "POST",
    headers: {
...
async function getCommentsByPostId(postId) {
  const token = getTokenFromCookie();
  if (!token) throw new Error("Token tidak ditemukan. Silakan login ulang.");
  const res = await fetch(`http://localhost:5000/forum/comments/post/${postId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  ...
  useEffect(() => {
    async function fetchPosts() {
      ...
      try {
        ...
        const url = `http://localhost:5000/forum/posts${params.length ? "?" + params.join("&") : ""}`;
