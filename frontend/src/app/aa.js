...
export default function ListLowonganPage() {
  ...
  const dummyLowongan = [
    ...
    },
    {
      ...
      status: "closed",
      kualifikasi: "S1 Teknik Komputer",
      jumlah_pelamar: 8,
      traffic: 80,
      batas_lamaran: "2024-06-30",
  ...
  return (
    <div className="w-full font-sans rounded-b-lg h-auto flex flex-col" style={{height: "100vh"}}>
      <div className="text-[20px] text-[#222] font-semibold mb-4 px-6 pt-6">
        Semua lowongan <span className="text-[#6c757d] font-normal">({dummyLowongan.length} total)</span>
      </div>
      <div
        className="w-full flex-1 overflow-y-auto pb-4 px-0"
        style={{ minHeight: 0 }}
      >
        {dummyLowongan.map((item) => {
          const isSaved = false;
          return (
            <div
              key={item._id}
            ...
            >
              ...
              <div className="flex-1 flex flex-col justify-between min-w-0">
                ...
                {/* Info bawah: pelamar, views, batas, waktu posting */}
                <div className="flex flex-wrap items-center gap-x-6 gap-y-1 mt-2">
                  ...
                  <span className="flex items-center text-[14px] text-[#6c757d] font-normal">
                    ...
                    Batas: <span className="ml-1 font-semibold text-[#222]">{item.batas_lamaran}</span>
                  </span>
                  <span className="flex items-center text-[14px] text-[#6c757d] font-normal">
                    <svg className="mr-1" width="15" height="15" fill="none" viewBox="0 0 20 20">
