    ...
    const fetchDetail = async () => {
      ...
      try {
        const res = await fetch(
          `https://tugasakhir-production-6c6c.up.railway.app/admin/lowongan/preview?id=${encodeURIComponent(lowonganId)}`,
          {
            method: "GET",
            headers: {
...
export default function VerifikasiLowonganPage() {
  ...
  useEffect(() => {
    const fetchPendingLowongan = async () => {
      ...
      try {
        const res = await fetch("https://tugasakhir-production-6c6c.up.railway.app/lowongan/pending/admin", {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleApprove = async (lowonganId) => {
    ...
    try {
      const res = await fetch(
        `https://tugasakhir-production-6c6c.up.railway.app/lowongan/${lowonganId}/verifikasi`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
